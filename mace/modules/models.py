...
@compile_mode("script")
class ScaleShiftLBMACE(torch.nn.Module):
    def __init__(
        self,
        r_mid: float,
        r_max: float,
        ...
        self.register_buffer("r_mid", torch.tensor(r_mid, dtype=torch.float64))
        self.register_buffer("r_max", torch.tensor(r_max, dtype=torch.float64))
        self.register_buffer(
            "num_interactions", torch.tensor(num_interactions, dtype=torch.int64)
        )
        ...
        self.radial_embedding_long = RadialEmbeddingBlock(
            r_max=r_max,
            num_bessel=num_bessel,
            num_polynomial_cutoff=num_polynomial_cutoff,
        )
        ...
        sh_irreps_long = o3.Irreps.spherical_harmonics(max_ell_nonbond)
        self.spherical_harmonics_long = o3.SphericalHarmonics(
            sh_irreps_long, normalize=True, normalization="component"
        )
        ...
        inter = interaction_cls_first(
            ...
            hidden_irreps=hidden_irreps,
            avg_num_neighbors=avg_num_neighbors,
            avg_num_neighbors_long=avg_num_neighbors_long,
            max_ell_nonbond=self.max_ell_nonbond,
            node_feats_irreps_long=self.node_feats_irreps_long,
        )
        self.interactions = torch.nn.ModuleList([inter])
    ...
    def forward(
        ...
        # Embeddings
        node_feats = self.node_embedding(data["node_attrs"])
        vectors_long, lengths_long = get_edge_vectors_and_lengths(
            positions=data["positions"],
            edge_index=data["edge_index"],
            shifts=data["shifts"],
        )
        rmid_mask = lengths_long.squeeze(1) < self.r_mid.item()
        edge_attrs_long = self.spherical_harmonics_long(vectors_long)
        edge_feats_long = self.radial_embedding_long(lengths_long)
        edge_index = data["edge_index"][:, rmid_mask].contiguous()
        edge_attrs = self.spherical_harmonics(vectors_long[rmid_mask])
        edge_feats = self.radial_embedding(lengths_long[rmid_mask])
        ...
        for interaction, product, readout in zip(
            ...
            node_feats, sc = interaction(
                ...
                edge_attrs_long=edge_attrs_long,
                edge_feats_long=edge_feats_long,
                edge_index_long=data["edge_index"],
            )
            node_feats = product(
                node_feats=node_feats,
                sc=sc,
        ...
        # Sum over interactions
        node_inter_es = torch.sum(
            torch.stack(node_energies_list, dim=0), dim=0
        )  # [n_nodes, ]
        node_inter_es = self.scale_shift(node_inter_es)
