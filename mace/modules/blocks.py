...
@compile_mode("script")
class MsInteractionBlock(InteractionBlock):
    ...
    def __init__(
        ...
        hidden_irreps: o3.Irreps,
        avg_num_neighbors: float,
        avg_num_neighbors_long: Optional[float] = None,
        max_ell_nonbond: Optional[int] = 1,
        node_feats_irreps_long: Optional[o3.Irreps] = None,
    ) -> None:
        super().__init__()
        self.node_attrs_irreps = node_attrs_irreps
        self.node_feats_irreps = node_feats_irreps
        ...
        self.hidden_irreps = hidden_irreps
        self.avg_num_neighbors = avg_num_neighbors
        self.avg_num_neighbors_long = avg_num_neighbors_long
        self.max_ell_nonbond = max_ell_nonbond
        self.node_feats_irreps_long = node_feats_irreps_long
    ...
    def forward(
        ...
        edge_feats: torch.Tensor,
        edge_index: torch.Tensor,
        edge_attrs_long: torch.Tensor,
        edge_feats_long: torch.Tensor,
        edge_index_long: torch.Tensor,
    ) -> torch.Tensor:
        raise NotImplementedError


@compile_mode("script")
class RealAgnosticResidualInteractionBlock_L(MsInteractionBlock):
    def _setup(self) -> None:
        # First linear
        self.linear_up = o3.Linear(
            self.node_feats_irreps,  # self.target_irreps,  self.node_feats_irreps,
            self.node_feats_irreps,
            internal_weights=True,
            shared_weights=True,
        )
        ...
        ############# long #############
        self.edge_attrs_irreps_long = o3.Irreps.spherical_harmonics(
            self.max_ell_nonbond.item()
        )
        # long-interaction
        self.linear_long_down = o3.Linear(
            self.node_feats_irreps,  # irreps_out  node_feats_irreps
            self.node_feats_irreps_long,
            internal_weights=True,
            shared_weights=True,
        )

        irreps_mid_long, instructions_long = tp_out_irreps_with_instructions(
            self.node_feats_irreps_long,
            self.edge_attrs_irreps_long,
            self.target_irreps,
        )
        self.conv_tp_long = o3.TensorProduct(
            self.node_feats_irreps_long,
            self.edge_attrs_irreps_long,
            irreps_mid_long,
            instructions=instructions_long,
            shared_weights=False,
            internal_weights=False,
        )
        self.conv_tp_weights_long = nn.FullyConnectedNet(
            [input_dim] + 3 * [64] + [self.conv_tp_long.weight_numel],
            self.activation,
        )
        self.linear_long_up = o3.Linear(
            irreps_mid_long.simplify(),
            self.irreps_out,
            internal_weights=True,
            shared_weights=True,
        )
    ...
    def forward(
        ...
        sc = self.skip_tp(node_feats, node_attrs)
        # ########## long ##########
        message_long = self.linear_long_down(node_feats)
        tp_weights_long = self.conv_tp_weights_long(edge_feats_long)
        mji_long = self.conv_tp_long(
            message_long[edge_index_long[1]],
            edge_attrs_long,
            tp_weights_long,
        )  # [n_edges, irreps]
        message_long = scatter_sum(
            src=mji_long, index=edge_index_long[0], dim=0, dim_size=num_nodes
        )  # [n_nodes, irreps]
        message_long = self.linear_long_up(message_long) / self.avg_num_neighbors_long
        #################################
        node_feats = self.linear_up(node_feats)
        tp_weights = self.conv_tp_weights(edge_feats)
        mji = self.conv_tp(
            node_feats[edge_index[1]], edge_attrs, tp_weights
        )  # [n_edges, irreps]
        message = scatter_sum(
            src=mji,
            index=edge_index[0],
        ...
        message = self.linear(message) / self.avg_num_neighbors
        message = (message + message_long) * 0.5
